...
export default ({ req, res, config, db }) => ({
  login() {
    if (!req.body.email)
      return toRes(res, 500)({ message: 'Email field is required!' });
    if (!req.body.password)
      return toRes(res, 500)({ message: 'Password field is required!' });
    UserModel(db).findOne(
      ...
      (err, user) => {
        ...
        if (!passwordIsValid)
          return toRes(res, 401)({ message: 'Invalid credentials!' });
        const token = jwtSign(
          {
            _id: user._id,
            name: user.name,
